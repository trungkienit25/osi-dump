...
class RoleAssignmentBatchHandler(IBatchHandler):
    @property
    def resource_name(self) -> str:
        return "role_assignment"

    def is_enabled(self, config: AppConfig) -> bool:
        return config.resources.role_assignment_enabled
    ...
    def process(self, connections: List[Any], output_dir: Path) -> Path:
        ...
        def generator():
            for conn in connections:
                yield from OpenStackRoleAssignmentImporter(conn).fetch_data()
