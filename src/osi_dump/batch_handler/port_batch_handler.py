...
from osi_dump.importer.port.openstack_port_importer import OpenStackPortImporter
from osi_dump.exporter.excel_exporter import PandasExcelExporter

@HandlerRegistry.register
class PortBatchHandler(IBatchHandler):
    @property
    def resource_name(self) -> str:
        return "port"

    def is_enabled(self, config: AppConfig) -> bool:
        return config.resources.port_enabled
    ...
    def process(self, connections: List[Any], output_dir: Path) -> Path:
        ...
        def generator():
            for conn in connections:
                yield from OpenStackPortImporter(conn).fetch_data()
